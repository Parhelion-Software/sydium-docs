...
| `draft_carousel` | `posts:write` | Draft a carousel from slide text; the user reviews the rendered slides in Sydium and schedules |
...
**Agencies:** call `list_clients` to get your clients, then pass a client's `id` as the `clientId` argument to `publish_post`, `draft_carousel`, `list_accounts`, or `list_scheduled_posts` to act on that client's connected accounts instead of your team's own.
